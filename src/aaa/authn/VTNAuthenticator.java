 ...
 * @since 0.1
 */
public class VTNAuthenticator extends ModularRealmAuthenticator {
    ...
    protected void assertRealmsConfigured() throws IllegalStateException {
        ...
        if (realms.size()!=1) {
            String msg = "Configuration error:  No or more than 1 realms have been configured!  Only one realm can be " +
...
//        if(!(authenticationToken instanceof VTNAuthNToken)){
//            throw new AuthenticationException("Unrecognized token , not a typeof TenantAuthenticationToken ");
//        } else {
//            MySQLRealm realm = (MySQLRealm) getRealms().iterator().next();
//            VTNAuthNToken tat = (VTNAuthNToken) authenticationToken;
//            String domID = Integer.toString(tat.getDomainId());
//            String user = tat.getUsername();
//            if(realm.getUserDomain(user).contains(domID)) {
//                return doSingleRealmAuthentication(realm, authenticationToken);
//            } else {
//                throw new AuthenticationException("Domain not found , not a valid domain requested ");
//            }
//        }
//    }
    ...
    protected AuthenticationInfo doAuthenticate(AuthenticationToken authenticationToken) throws AuthenticationException {
        ...
        if(!(authenticationToken instanceof VTNAuthNToken)){
            ...
            MySQLRealm realm = (MySQLRealm) getRealms().iterator().next();
            VTNAuthNToken token = (VTNAuthNToken) authenticationToken;
